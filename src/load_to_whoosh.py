            ...
            for row in cursor.fetchall():
                try:
                    ...
                    doc = {
                        ...
                        "currency": product.price_info.currency_code if product.price_info else None,
                        "color_families": ",".join(product.color_info.color_families or []) if product.color_info else None,
                        "colors": ",".join(product.color_info.colors or []) if product.color_info else None,
                        "categories": ",".join(cat.name for cat in (product.categories or [])),
                        "rating": product.rating.average_rating if product.rating else None,
                        "review_count": product.rating.rating_count if product.rating else None,
                        "image_url": product.image.url if product.image else None,
                        "image_urls": [img.url for img in product.images] if product.images else None
                    }
                    
                    writer.add_document(**{k: v for k, v in doc.items() if v is not None})
                    
                except Exception as e:
                    print(f"Error processing product: {e}")
                    continue

            print(f"Indexed {min(offset + batch_size, total_rows)} of {total_rows} products")
