...
def load_to_sqlite(conn: sqlite3.Connection, parquet_files: list[str], table_name: str) -> tuple[int, int]:
...
def load_to_duckdb(conn: duckdb.DuckDBPyConnection, parquet_files: list[str], table_name: str) -> tuple[int, int]:
    ...
    if parquet_files:
        ...
        logger.info(f"Created table {table_name} with schema from first file ({first_count} products)")
...
def load_parquet_files_to_db(
    ...
    db_path = os.path.join(os.path.dirname(__file__), "..", f"{catalog}_catalog.{db_type}")
    ...
    try:
        if db_type == "sqlite":
            total_products, duplicates_skipped = load_to_sqlite(conn, parquet_files, table_name)
        else:  # duckdb
            total_products, duplicates_skipped = load_to_duckdb(conn, parquet_files, table_name)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Load Octogen catalog data to SQLite or DuckDB")
    parser.add_argument(
        "--catalog",
        type=str,
