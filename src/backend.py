...
async def search_products(
    ...
    # Get full product data from database using productGroupID
    conn = get_db_connection(table_name)
    ...
    if isinstance(conn, duckdb.DuckDBPyConnection):
        products = cursor.execute(
            f"SELECT extracted_product FROM {table_name} WHERE json_extract_string(extracted_product, '$.productGroupID') IN ({placeholders})",
            product_ids,
        ).fetchall()
    else:
        products = cursor.execute(
            f"SELECT extracted_product FROM {table_name} WHERE json_extract(extracted_product, '$.productGroupID') IN ({placeholders})",
            product_ids,
        ).fetchall()
