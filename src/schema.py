...
def to_native_type(value: Any) -> Any:
    """Recursively convert Pydantic models and HttpUrl objects to native types."""
    if isinstance(value, BaseModel):
        return {k: to_native_type(v) for k, v in value.dict().items()}
...
def deserialize_row(row: Any) -> Any:
    """Recursively convert ndarray to list in a nested dictionary or list."""
    if isinstance(row, dict):
        return {k: deserialize_row(v) for k, v in row.items()}
    elif isinstance(row, list):
        return [deserialize_row(v) for v in row]
...
class FacetSearch(BaseModel):
    name: str
    value: str


class FacetSearchList(BaseModel):
    facets: List[FacetSearch]
...
class SearchProduct(BaseModel):
    ...
    # The URL of where the product was crawled, which may be different from the product URL itself
    crawl_url: HttpUrl
    # The product URL
    # @todo: Maybe this should / could be a function that is configured per tenant
...
class ProductRecommendation(SearchProduct):
    justification: str


class ProductRecommendationBase:
    uuid: str
    justification: str


class ProductBase(SearchProduct):
    crawl_timestamp: int
    catalog: str
    crawl_page_content: str
    embedding: List[float] = [0] * EMBEDDING_DIMENSION
    name_embedding: List[float] = [0] * EMBEDDING_DIMENSION
    description_embedding: List[float] = [0] * EMBEDDING_DIMENSION
    keywords_embedding: List[float] = [0] * EMBEDDING_DIMENSION
    image_captions_embedding: List[float] = [0] * EMBEDDING_DIMENSION
    facets: List[str] = []
    ...
    def check_embedding_length(cls, v: List[float]) -> List[float]:
        if len(v) != EMBEDDING_DIMENSION:
            raise ValueError(
                "embedding must have exactly {} elements".format(EMBEDDING_DIMENSION)
            )
        return v
